...
function evenSum(num) {
    ...
    // function performing this particular fibonacci sequence
    for (let i = 1; i <= num; i++) {
        // Adding numbers to the total sum
        let totalSum = currentNum + nextNum;

        // Set the next number to the next number
        currentNum = nextNum;

        // Adding next number to total sum
        nextNum = totalSum;

        //Now we are going to make sure these numbers that are displayed are evensum
        if (totalSum % 2 == 0) {
            sum += totalSum;
        }
    }
...
// while the number is greater then 1, see if the divisble number goes into the number
while(number > 1){
    if(number % divisor === 0){ 
        number /= divisor;
    } else {
...
return (
    <>
    <h1>This is the practice page for euler. Look in the console for some code!</h1>
    </>
)
}
